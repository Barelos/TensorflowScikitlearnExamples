"""
Work with iris dataset using scikit-learn and tensorfow
"""
import numpy as np
...
clf_tree.fit(tr[:,:-1], tr[:,-1])
predictions_tree = clf_tree.predict(te[:,:-1])
# KNN
from sklearn.neighbors import KNeighborsClassifier
clf_knn = KNeighborsClassifier()
clf_knn.fit(tr[:,:-1], tr[:,-1])
predictions_knn = clf_knn.predict(te[:,:-1])
# neural net
import tensorflow as tf

name_to_vec= {"Iris-versicolor" : np.array([1,0,0]),
              "Iris-virginica" : np.array([0,1,0]),
...
weights = {
        ...
        'out' : tf.Variable(tf.random_normal([n_h2, n_class]))
}
biases = {
...
with tf.Session() as sess:
    sess.run(init)
    batch_x, batch_y = tr[:,:-1], [name_to_vec[i] for i in tr[:,-1]]
    for i in range(100):
        sess.run(train_op, feed_dict={X:batch_x, Y:batch_y})
    # Calculate accuracy
    accuracy_nn = sess.run(accuracy, feed_dict={X: te[:,:-1], Y: [name_to_vec[i] for i in te[:,-1]]})

# show results
print("Tree:", accuracy_score(te[:,-1], predictions_tree))
print("KNN:", accuracy_score(te[:,-1], predictions_knn))
